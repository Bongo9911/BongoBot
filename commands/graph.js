            ...
            for (let i = 0; i < items.length; ++i) {
                datasets.push({
                    ...
                    borderColor: items[i].color.length ? ['rgba(' + items[i].color[0] + "," + items[i].color[1] + "," + items[i].color[2] + ", 1)"] :
                        ['rgba(' + Math.round(colors[i % colors.length][0] / (1 + Math.min(i / colors.length))) + ',' + Math.round(colors[i % colors.length][1] / (1 + Math.min(i / colors.length)))
                            + ',' + Math.round(colors[i % colors.length][2] / (1 + Math.min(i / colors.length))) + ', 1)'],
                    backgroundColor: items[i].color.length ? 'rgba(' + items[i].color[0] + "," + items[i].color[1] + "," + items[i].color[2] + ", 1)" :
                        'rgba(' + Math.round(colors[i % colors.length][0] / (1 + Math.min(i / colors.length))) + ',' + Math.round(colors[i % colors.length][1] / (1 + Math.min(i / colors.length)))
                        + ',' + Math.round(colors[i % colors.length][2] / (1 + Math.min(i / colors.length))) + ', 1)' //0.2
                })
            }
